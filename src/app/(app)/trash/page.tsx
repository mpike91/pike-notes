'use client';

import { useEffect } from 'react';
import { AppHeader } from '@/components/layout/AppHeader';
import { NotesList } from '@/components/notes/NotesList';
import { useNotes } from '@/hooks/use-notes';

export default function TrashPage() {
  const { notes, isLoading, fetchNotes, setFilter } = useNotes();

  useEffect(() => {
    setFilter('trashed');
    fetchNotes();
    return () => setFilter('all');
  }, [setFilter, fetchNotes]);

  return (
    <>
      <AppHeader title="Trash" />
      <div className="flex-1 overflow-y-auto">
        {!isLoading && notes.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-center px-4">
            <svg className="h-12 w-12 text-text-muted" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" />
            </svg>
            <h2 className="mt-4 text-lg font-medium text-text-primary">Trash is empty</h2>
            <p className="mt-1 text-sm text-text-muted">Items in trash are automatically deleted after 30 days.</p>
          </div>
        ) : (
          <NotesList notes={notes} isLoading={isLoading} />
        )}
      </div>
    </>
  );
}
